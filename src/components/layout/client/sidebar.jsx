import { navigationClient } from "@/data/navigation-sidebar";
...
const SidebarSection = ({ isOpen, closeSidebar }) => {
  ...
  const filteredNavigation = navigationClient.filter((item) => {
              ...
              <div key={item.name} className="space-y-1">
                {item.hasDropdown ? (
                  <button
                    ...
                    className={`group flex w-full cursor-pointer items-center gap-2 p-1 text-base font-medium transition-all duration-200 ${
                      isParentActive(item)
                        ? "text-primary-dark"
                        : "text-dark-heading"
                    }`}
                  >
                    ...
                    <ChevronRight
                      className={`h-4 w-4 transition-transform duration-200 ${
                        shouldDropdownBeOpen(item) ? "rotate-90" : ""
                      }`}
                    />
                  </button>
                ) : (
                  <Link
                    ...
                    className={`group flex w-full items-center gap-2 p-1 text-base font-medium transition-all duration-200 ${
                      isItemActive(item)
                        ? "text-primary-dark"
                        : "text-dark-heading"
                    }`}
                      ...
                      <div className="ml-6 space-y-1 border-l border-gray-200 pl-4">
                        {item.children?.map((child) => (
                          <Link
                            ...
                            className={`group flex items-center gap-2 p-1 text-sm font-medium transition-all duration-200 ${
                              isItemActive(child)
                                ? "text-primary-dark"
                                : "hover:text-dark-heading text-gray-600"
                            }`}
                          >
                            {child.name}
                          </Link>
